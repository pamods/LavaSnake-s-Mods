...
function lockFrame(id) {
	...
	localStorage["frames_" + id + "_lockStatus"] = true;
}

function unlockFrame(id) {
	$("#" + id).draggable({ disabled: false });
	localStorage["frames_" + id + "_lockStatus"] = false;
}
...
function createFloatingFrame(id, width, height, options) {
	...
	if (!localStorage["frames_" + id + "_lockStatus"]) {
		localStorage["frames_" + id + "_lockStatus"] = false;
	}
	if (localStorage["frames_" + id + "_lockStatus"] == "true") {
		$("#" + id).draggable({ disabled: true });
	}
}